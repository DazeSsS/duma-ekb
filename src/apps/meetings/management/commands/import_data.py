...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        filename = 'Протколы XIX век.xlsx'
        workbook = load_workbook(settings.DATA_PATH / filename, read_only=True)
        ...
        for idx, row in enumerate(rows):
            ...
            try:
                ...
                self.stdout.write(self.style.ERROR(f'Не удалось обработать строку {idx + 2} в файле "{filename}": {e}'))
